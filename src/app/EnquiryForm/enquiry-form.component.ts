...
import { UserDataService } from '../Enquiryservices/user-data.service';
...
export class EnquiryFormComponent  implements OnInit, AfterContentChecked {
  ...
  onSave(): void {
    this.postFormData('https://enquirerwebapi.azurewebsites.net/api/SaveEnquiry');
    
  }

  onSubmit(): void {
    this.postFormData('https://enquirerwebapi.azurewebsites.net/api/CreateEnquiry');
  ...
  const model = {
   ...
   const loadingSnackbarRef = this.snackBar.open('Submitting data, please wait...', 'Close', {
    duration: 3000 // 0 means it will stay until dismissed
  });
    ...
    this.http.post(apiUrl, body, {headers:headers})
      .subscribe(
        response => {
          ...
          this.postFormData2('https://enquirerwebapi.azurewebsites.net/api/CreateDocuments');
